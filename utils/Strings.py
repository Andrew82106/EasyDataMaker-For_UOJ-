...
def GenerateWord(lengthRange: tuple, charset: str):
    Str = String.random(lengthRange, charset=charset)
    print(Str)
    redirectOutput([[Str]])
    return [[Str]]


def GenerateSentences(lengthRange: tuple, Charset: str, First_letter_uppercase=False):
    Str = String.random_sentence(lengthRange, word_separators=" ",
                                 first_letter_uppercase=First_letter_uppercase, word_length_range=lengthRange,
                                 charset=Charset)
    print(Str)
    res = [[str(len(Str.split(" ")))], [""]]
    for i in Str.split(" "):
        res[-1][0] += i + " "
    redirectOutput(res)
    return res
...
if __name__ == '__main__':
    print(GenerateSentences((1, 10), "1314wq", 0))
    print(type(GenerateWord((1, 10), "1314wq")))